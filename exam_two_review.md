...
* __Interrupt context__: this is invoked by the spontaneous change in the flow of execution when a hardware interrupt takes place.  
...
* __User context__: this is the flow of control when a user process invokes a system call.  
...
* __Kernel context__: the kernel itself has one or more worker threads that is schedules just like any other process.  
  - May be blocked.

## Device Drivers

To ensure that the interrupt context does not take too long (due to unable to block), it is split into two parts:  
* The __top half__ of a driver is the interrupt service routine that is registered with the interrupt handler.  
  - It tries to do as little as possible -- generally grabbing data, placing it into a __work queue__ (a buffer).
* The __bottom half__ of a driver is the part that is   scheduled by the top half for later execution.  
